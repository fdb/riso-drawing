...
# usage: ./render-anim.sh [frames] [seed] [scene]
N=${1:-56}; S=${2:-5}; SC=${3:-jelly}
...
while [ $i -lt $N ]; do
  ...
  "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless=new --disable-gpu --hide-scrollbars --allow-file-access-from-files \
    --virtual-time-budget=20000 --window-size=1080,1080 --screenshot="renders/frames/f_$f.png" "file://$PWD/render.html?scene=$SC&f=$i&s=$S" >/dev/null 2>&1
  i=$((i+1))
done

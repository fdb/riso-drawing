...
/** Renders the root scene every frame. Paused frames re-render only when the document or view changes. */
export function Viewport() {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      const { doc, ui } = st;
      if (!runner || runner.res !== ui.res) {
        cv.width = cv.height = ui.res;
        runner = new SceneRunner(ui.res, doc.scene);
      }
      if (lastLib !== doc.lib) {
        setLibrary(doc.lib);
        lastLib = doc.lib;
      }
      runner.scene = doc.scene;
        ...
        try {
          ...
          if (hudRef.current)
            hudRef.current.textContent =
              `t ${time.toFixed(2)}s  frame ${frame}  ${phaseName(time, doc.scene.transition, ui.loopHold)}  iris ${info.iris.toFixed(2)}\n` +
